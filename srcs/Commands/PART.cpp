	...
	for (size_t i = 0; i < chanName.size(); i++) {
		if ((index = _channelExist(chanName[i])) >= 0) {
			if (_channels[index].checkClientConnected(_clients[actualClient]) >= 0) {
				if (!info.empty())
					_channels[index].sendToAllChannel(RPL_PART_MESSAGE(_clients[actualClient].createClientPrompt(), _channels[index].getChannelName(), info));
				else
					_channels[index].sendToAllChannel(RPL_PART_NOMESSAGE(_clients[actualClient].createClientPrompt(), _channels[index].getChannelName()));
